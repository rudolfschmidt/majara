package com.rudolfschmidt.majara.v1.compiler;
	...
	private void compileElementTextValue(String text, StringBuilder sb) {
		Optional.ofNullable(text).ifPresent(value -> {
			...
			while (matcher.find()) {
				...
				Object mod = parseModel(key);
				value = value.replaceAll("#\\{" + key + "\\}", Optional.ofNullable(mod)
						.map(Object::toString).orElse(matcher.group()));
			}
			sb.append(value);
		});
	}
